...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        ghostPosition = None
        for _distribution in livingGhostPositionDistributions:
            _position = _distribution.argMax()
            _distance = self.distancer.getDistance(pacmanPosition, _position)
            if _distance < ghostDistance:
                ghostDistance = _distance
                ghostPosition = _position

        ghostDistance = float('inf')
        action = None
        for _action in legal:
            nextPosition = Actions.getSuccessor(pacmanPosition, _action)
            _distance = self.distancer.getDistance(nextPosition, ghostPosition)
            if _distance < ghostDistance:
                ghostDistance = _distance
                action = _action
        return action
