...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for next_state, probability in self.mdp.getTransitionStatesAndProbs(state, action):
            ...
            ret += probability * (reward + self.discount * self.values[next_state])
        return ret
        ...
        for _ in range(self.iterations):
            ...
            if not self.mdp.isTerminal(state):
                self.values[state] = max(
                    [self.getQValue(state, action) for action in self.mdp.getPossibleActions(state)])
            for prev_state in predecessors[state]:
                diff = abs(self.values[prev_state] - max(
                    [self.getQValue(prev_state, action) for action in self.mdp.getPossibleActions(prev_state)]))
                if diff > self.theta:
                    pq.update(prev_state, -diff)
